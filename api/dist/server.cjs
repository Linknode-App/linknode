...
function toHumanReadable(iso, zone = "utc") {
  const safeIso = iso || luxon.DateTime.now().toUTC().toISO({ suppressMilliseconds: true });
  return luxon.DateTime.fromISO(safeIso, { zone }).toFormat("cccc, LLLL d, yyyy, t ZZZZ");
}
...
`;
  try {
    ...
    if (start !== -1 && end !== -1) {
      ...
      return arr.map((tweet) => {
        const iso = tweet.timestamp || request.current_time || luxon.DateTime.now().toUTC().toISO({ suppressMilliseconds: true });
        return {
          text: tweet.text,
          timestamp: iso,
  ...
  return tweets.map((t) => {
    const iso = t.timestamp || request.current_time || luxon.DateTime.now().toUTC().toISO({ suppressMilliseconds: true });
    return {
      text: t.text,
      timestamp: iso,
      readable_time: toHumanReadable(iso, audience_timezone || "utc")
